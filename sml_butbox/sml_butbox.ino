...
// 0 disabled, > 0 enabled
#define WS2812B_RGBLEDCOUNT 3       //{"Group":"WS2812B RGB Leds","Name":"WS2812B_RGBLEDCOUNT","Title":"WS2812B RGB leds count","DefaultValue":"0","Type":"int","Max":150}
...
byte rowPins[ROWS] = {8, 9, 5, 6, 2}; //connect to the row pinouts of the keypad
...
void pin_init(void)
{
  ...
  DDRD |= (1<<PD5);    //Configure PORTD pin 5 as an Output
...
void read_button_matrix(void)
{
  char keych;
    ...
    if (keypad.getKeys())
    {         
        for (int j=0; j<20; j++)   // Scan the whole key list.
        {
